class Room {
  ...
  Room(float xPos, float yPos) {
    ...
    if (chestSpawn <= 50) {
      Chest c = new Chest(xPos + random(0, w - 90), yPos + random(0, h- 95));
      scenePlay.chests.add(c);
    }
    ...
    w2.identifier = 1;
    d1 = new Door(x + w/2 - gapSize/4, y + wallThickness/2, wallThickness, gapSize);
    ...
    w4.identifier = 2;
    d2 = new Door(w1.x + w/2 + w1.w/2, y + h/2 - gapSize/4, gapSize, wallThickness);
    ...
    w8.identifier = 3;
    d4 = new Door(w1.x - platformWidth/2, y + h/2 - gapSize/4, gapSize, wallThickness);
    ...
    w6.identifier = 4;
    d3 = new Door(x + w/2 - gapSize/4, y + platformHeight*2 + gapSize/2, wallThickness, gapSize);
